...
class BotLogic:
    ...
    def initialize(self, player_name, map_radius, players, turns, home_base_positions):
        ...
        # Detectar compañeros
        self.has_leela = any("leela" in p.lower() for p in players if p != player_name)
        self.has_fry   = any("fry"   in p.lower() for p in players if p != player_name)

        # Punto fijo de Fry
        self.fry_anchor = Position(-1, 1)
        self.FRY_AVOID_RADIUS = 3   # radio de evasión para Leela
    ...
    def _explore_sextant(self, position, speed, radar_contacts):
        ...
        # Evitar zona Fry
        if "leela" in self.player_name.lower() and self.has_fry:
            if center.distance_to(self.fry_anchor) <= self.FRY_AVOID_RADIUS:
                # empujar el centro hacia afuera
                dx = center.x - self.fry_anchor.x
                dy = center.y - self.fry_anchor.y
                center = Position(center.x + dx, center.y + dy)
        free = self._safe_reachable(position, speed, radar_contacts)
        if not free:
            return None
    ...
    def _safe_reachable(self, position, speed, radar_contacts):
        ...
        for p in position.positions_in_range(speed):
            ...
            if d == 0 or d > speed:
                continue
            result.append(p)
        return self._avoid_fry_zone(result)
    ...
    def _avoid_fry_zone(self, positions):
        if not ("leela" in self.player_name.lower() and self.has_fry):
            return positions

        # solo evitar posiciones finales demasiado cercanas
        safe = [p for p in positions if p.distance_to(self.fry_anchor) >= 2]
