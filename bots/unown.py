...
class BotLogic:
    ...
    def _get_expanded_center(self, sextant_name, speed):
        ...
        if speed >= 3:
            # 2 hacia el borde en cada eje + 1 lateral (el eje más cercano al borde)
            border_step = 2
            lateral_step = 1
        else:
            # speed 2: solo 2 hacia el borde, sin lateral
            border_step = 2
            lateral_step = 0

        d = self._base_d + self.expansion_level * border_step
