...
class BotLogic:
    ...
    def turn(self, turn_number, hp, ship_number, cargo, position, power_distribution,
        ...
        # ── MODO FRY ─────────────────────────────────────────────
        if "fry" in self.player_name.lower() and self.has_leela:

            # Si no está en el anchor → moverse rápido
            if position != self.fry_anchor:
            ...
            # Calcular speed real
            speed = max(1, desired_power[ENGINES])

            # Ir al punto (-1,1)
            if position != self.fry_anchor:
                reachable = self._safe_reachable(position, speed, radar_contacts)
                if reachable:
                    return FLY_TO, min(reachable, key=lambda p: p.distance_to(self.fry_anchor))

            # Ya está en posición → quedarse
            return None
        if self.first_turn:
